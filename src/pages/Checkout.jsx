  ...
  if (!product) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <h2 className="text-xl font-semibold text-[#051a2f]">
          No product selected
        </h2>
      </div>
    );
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      await createOrder({
        ...
        price: numericPrice,
        quantity: qty,
        total: total,                   // schema requires total
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-lg p-8 w-[300px] text-center">
            ...
            <button onClick={() => { setShowSuccess(false); navigate("/cart"); }} className="mt-5 bg-[#051a2f] text-white px-5 py-2 rounded-lg text-sm">
              Continue Shopping
            </button>
          </div>
        </div>
